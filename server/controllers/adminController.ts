...
// crate delivery partner profile 
export const createtDeliveryPartner = async (req: Request, res: Response)  => {
...
export const updateDeliveryPartner = async (req: Request, res: Response)  => {
  ...
  try {
    ...
    res.status(400).json({message: "Partner does not found"})
...
export const assignDeliveryPartner = async (req: Request, res: Response)  => {
  ...
  if(order!.status === "Placed" || order!.status === "Confermed") {
    status = "Assigned";
    history.push({
      status: "Assigned",
